...
<template>
    ...
    <div class="min-h-screen flex text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-950 font-sans selection:bg-indigo-500 selection:text-white">
        ...
        <div class="flex flex-col justify-center w-full lg:w-1/2 px-8 sm:px-16 md:px-24 xl:px-32 relative z-10">
            <!-- Decorative Elements -->
            ...
            <div class="w-full max-w-sm mx-auto relative z-10">
                ...
                <div class="mb-10 flex flex-col items-center sm:items-start text-center sm:text-left">
                    <Link href="/" class="inline-block mb-6 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 rounded-xl">
                    ...
                    <p class="mt-2 text-sm text-slate-500 dark:text-slate-400 font-medium">
                        Masukkan kredensial Anda untuk melanjutkan ke platform cerdas kami.
                    </p>
                </div>

                <div v-if="status" class="mb-6 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-100 dark:border-emerald-800/50 text-sm font-bold text-emerald-600 dark:text-emerald-400">
                    {{ status }}
                </div>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="flex items-center justify-between mt-2">
                        <label class="flex items-center cursor-pointer group">
                            <Checkbox name="remember" v-model:checked="form.remember" class="rounded text-indigo-600 focus:ring-indigo-500 border-slate-300 dark:border-slate-600 dark:bg-slate-800" />
                            <span class="ms-2 text-sm font-medium text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-200 transition-colors">Ingat saya</span>
                        </label>

                        <Link
                            v-if="canResetPassword"
                            :href="route('password.request')"
                            class="text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 transition-colors focus:outline-none focus:underline"
                        >
                            Lupa password?
                        </Link>
                    </div>
                    ...
                    <div class="text-center mt-6">
                        <p class="text-sm text-slate-500 dark:text-slate-400 font-medium">
                            Belum punya akun?
                            <Link :href="route('register')" class="font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 transition-colors focus:outline-none focus:underline">
                                Daftar sekarang
                            </Link>
                        </p>
                    </div>
